...
{
    FILE *fp;
    fp = fopen("C:/Users/Juan Carlos/OneDrive/Documentos/Info 2/inform-tica-2/4. Arcrivos/Ejercicios/el_quijote.txt", "r");
	check_null(fp);

    char palabra[20];
	int cont_Quijote=0, cont_Rocinante=0, cont_Sancho=0;

    while(fscanf(fp,"%19s",palabra) != EOF)
	{
	...
	fclose(fp); // Cierra el archivo después de usarlo
    return 0;
}
